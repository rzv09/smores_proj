...
"""
In this file we only do an experiment with FL1,2
"""
class TransferLearningFL3FL4():
    def __init__(self, data_path_fl, num_train_epochs, sampling_freq, sequence_len, device):
    ...
    def process_data(self):
        ...
        # For training, use FL3, FL4 split
    ...
    def unnormalize(self, preds: torch.FloatTensor, ds_lbl: str):
        ...
        if ds_lbl == 'fl_dataset1':
            ...
            unnormed_labels = self.test_labels1_tensor.squeeze(1).cpu().numpy() * self.fl1_std + self.fl1_mean
        elif ds_lbl == 'fl_dataset2':
            y_hat = preds.numpy() * self.fl2_std + self.fl2_mean
            unnormed_labels = self.test_labels2_tensor.squeeze(1).cpu().numpy() * self.fl2_std + self.fl2_mean
        elif ds_lbl == 'fl_dataset3':
            y_hat = preds.numpy() * self.fl3_std + self.fl3_mean
            unnormed_labels = self.test_labels3_tensor.squeeze(1).cpu().numpy() * self.fl3_std + self.fl3_mean
        elif ds_lbl == 'fl_dataset4':
            y_hat = preds.numpy() * self.fl4_std + self.fl4_mean
            unnormed_labels = self.test_labels4_tensor.squeeze(1).cpu().numpy() * self.fl4_std + self.fl4_mean
        else:
