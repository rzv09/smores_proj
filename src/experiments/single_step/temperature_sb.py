...
class TemperatureSB():
    ...
    def process_data(self):
        ...
        # Temperature
        self.first_piece_T = process_sb.get_first_piece_T(self.df)
        self.first_piece_mean_T = self.first_piece_T.mean().values
        self.first_piece_std_T = self.first_piece_T.std().values
        self.second_piece_T = process_sb.get_second_piece_T(self.df)
        self.second_piece_mean_T = self.second_piece_T.mean().values
        self.second_piece_std_T = self.second_piece_T.std().values
        self.third_piece_T = process_sb.get_third_piece_T(self.df)
        self.third_piece_mean_T = self.third_piece_T.mean().values
        self.third_piece_std_T = self.third_piece_T.std().values
        self.fourth_piece_T = process_sb.get_fourth_piece_T(self.df)
        self.fourth_piece_mean_T = self.fourth_piece_T.mean().values
        self.fourth_piece_std_T = self.fourth_piece_T.std().values
        ...
        # create temperature sequences, ignore labels
        self.T_train_seq1, _ = create_sequences_single_step(standardize_piece(self.first_piece_T).values, self.sequence_len)
        self.T_train_seq2, _ = create_sequences_single_step(standardize_piece(self.second_piece_T).values, self.sequence_len)
        self.T_train_seq3, _ = create_sequences_single_step(standardize_piece(self.third_piece_T).values, self.sequence_len)
        self.T_train_seq4, _ = create_sequences_single_step(standardize_piece(self.fourth_piece_T).values, self.sequence_len)

        # create oxygen tensors
        self.Ox_seq1_tensor = make_sequence_tensor(self.train_seq1, self.device)
        self.labels1_tensor = make_sequence_tensor(self.train_labels1, self.device)
        self.Ox_seq2_tensor = make_sequence_tensor(self.train_seq2, self.device)
        self.labels2_tensor = make_sequence_tensor(self.train_labels2, self.device)
        self.Ox_seq3_tensor = make_sequence_tensor(self.train_seq3, self.device)
        self.labels3_tensor = make_sequence_tensor(self.train_labels3, self.device)
        self.Ox_seq4_tensor = make_sequence_tensor(self.train_seq4, self.device)
        self.labels4_tensor = make_sequence_tensor(self.train_labels4, self.device)

        # create temperature tensors
        self.T_seq1_tensor = make_sequence_tensor(self.T_train_seq1, self.device)
        self.T_seq2_tensor = make_sequence_tensor(self.T_train_seq2, self.device)
        self.T_seq3_tensor = make_sequence_tensor(self.T_train_seq3, self.device)
        self.T_seq4_tensor = make_sequence_tensor(self.T_train_seq4, self.device)

        # combine oxygen and temperature tensors into tensors of 2d input pairs [oxygen, temp]

        self.seq1_tensor = torch.stack((self.Ox_seq1_tensor, self.T_seq1_tensor), dim=1)
        self.seq2_tensor = torch.stack((self.Ox_seq2_tensor, self.T_seq2_tensor), dim=1)
        self.seq3_tensor = torch.stack((self.Ox_seq3_tensor, self.T_seq3_tensor), dim=1)
        self.seq4_tensor = torch.stack((self.Ox_seq4_tensor, self.T_seq4_tensor), dim=1)
    ...
    def train_model_es(self, num_epochs: int, model_label: str, patience: int = 10):
        ...
        model = lstm_single_step.create_lstm_single_step(input_dim=2, num_features=2, hidden_dim=128)
        model.to(self.device)
        optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
        criterion = nn.SmoothL1Loss()
