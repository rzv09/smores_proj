...
"""
Here we use only two clients during training
"""
class FederatedLearningSB3SB4():
    def __init__(self, data_path_sb, num_train_epochs, num_rounds,
    ...
    def process_data(self):
        ...
        # For training, use SB3, SB4 split
        ...
        # For testing, use SB1, SB2, SB3, SB4 partitions

        # create train sequences
        self.train_seq1, self.train_labels1 = create_sequences_single_step(standardize_piece(self.sb3).values, self.sequence_len)
        self.train_seq2, self.train_labels2 = create_sequences_single_step(standardize_piece(self.sb4).values, self.sequence_len)
        ...
        self.test_seq3, self.test_labels3 = create_sequences_single_step(standardize_piece(self.sb1).values, self.sequence_len)
        # self.test_seq2, self.test_labels2 = create_sequences_single_step(standardize_piece(self.fl1).values, self.sequence_len)
        self.test_seq4, self.test_labels4 = create_sequences_single_step(standardize_piece(self.sb2).values, self.sequence_len)
        # self.test_seq4, self.test_labels4 = create_sequences_single_step(standardize_piece(self.fl2).values, self.sequence_len)

        # sb 3
        self.test_seq1_tensor = make_sequence_tensor(self.train_seq1, self.device)
        self.test_labels1_tensor = make_sequence_tensor(self.train_labels1, self.device)
        # sb 4
        self.test_seq2_tensor = make_sequence_tensor(self.train_seq2, self.device)
        self.test_labels2_tensor = make_sequence_tensor(self.train_labels2, self.device)
        # sb 1
    ...
    def unnormalize(self, preds: torch.FloatTensor, ds_lbl: str):
        ...
        preds = preds.cpu()
        # the following means and stds are picked CORRECTLY; the index is not mixed up
        ...
        elif ds_lbl == 'sb_dataset1':
            y_hat = preds.numpy() * self.sb1_std + self.sb1_mean
            unnormed_labels = self.test_labels3_tensor.squeeze(1).cpu().numpy() * self.sb1_std + self.sb1_mean
        elif ds_lbl == 'sb_dataset4':
            y_hat = preds.numpy() * self.sb4_std + self.sb4_mean
            unnormed_labels = self.test_labels2_tensor.squeeze(1).cpu().numpy() * self.sb4_std + self.sb4_mean
        elif ds_lbl == 'sb_dataset2':
            y_hat = preds.numpy() * self.sb2_std + self.sb2_mean
            unnormed_labels = self.test_labels4_tensor.squeeze(1).cpu().numpy() * self.sb2_std + self.sb2_mean
        else:
    ...
    def set_test_data(self, model_lbl: str, test_only: bool=False):
        self.dataset_lbls = ['sb_dataset3', 'sb_dataset4', 'sb_dataset1', 'sb_dataset2']
