...
class CrossRegionSB4FL12():
    ...
    def process_data(self):
        ...
        # create train sequences (SB1-4, FL1-2)
        ...
        self.train_seq3, self.train_labels3 = create_sequences_single_step(standardize_piece(self.sb3).values, self.sequence_len)
        self.train_seq4, self.train_labels4 = create_sequences_single_step(standardize_piece(self.sb4).values, self.sequence_len)
        self.train_seq5, self.train_labels5 = create_sequences_single_step(standardize_piece(self.fl1).values, self.sequence_len)
        self.train_seq6, self.train_labels6 = create_sequences_single_step(standardize_piece(self.fl2).values, self.sequence_len)

        # create TEST (FL3-4) sequences
        self.fl_seq3, self.fl_labels3 = create_sequences_single_step(standardize_piece(self.fl3).values, self.sequence_len)
        self.fl_seq4, self.fl_labels4 = create_sequences_single_step(standardize_piece(self.fl4).values, self.sequence_len)
    ...
    def unnormalize(self, preds: torch.FloatTensor, ds_lbl: str):
        ...
        if ds_lbl == 'fl_dataset3':
            ...
            unnormed_labels = self.fl_labels3_tensor.squeeze(1).cpu().numpy() * self.fl3_std + self.fl3_mean
        elif ds_lbl == 'fl_dataset4':
            y_hat = preds.numpy() * self.fl4_std + self.fl4_mean
            unnormed_labels = self.fl_labels4_tensor.squeeze(1).cpu().numpy() * self.fl4_std + self.fl4_mean
        else:
    ...
    def set_test_data(self, test_only: bool=False):
        ...
        #     return True
        self.dataset_lbls = ['fl_dataset3', 'fl_dataset4']
        self.test_seqs = [self.fl3_tensor, self.fl4_tensor]
        self.test_lbls = [self.fl_labels3_tensor, self.fl_labels4_tensor]
