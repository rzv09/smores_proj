...
class CrossRegionFederatedForgettingFedProx():
    def __init__(self, data_path_sb, data_path_fl, num_train_epochs, num_rounds,
                 mu, sampling_freq, sequence_len, device):
    ...
    def _prox_term(self, local_model: nn.Module, global_model: nn.Module):
        prox = 0.0
        for local_param, global_param in zip(local_model.parameters(), global_model.parameters()):
            prox += torch.norm(local_param - global_param.to(local_param.device)) ** 2
        return 0.5 * self.mu * prox
            ...
            for label in client_labels:
                ...
                for _ in range(self.num_train_epochs):
                    ...
                    for j in range(len(seq)):
                        ...
                        loss = criterion(pred, lbls[j]) + self._prox_term(local_model, global_model)
                        loss.backward()
                        optimizer.step()
