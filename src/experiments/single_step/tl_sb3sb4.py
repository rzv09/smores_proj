...
"""
In this file we only do an experiment with SB1,2,3,4
"""
class TransferLearningSB3SB4():
    def __init__(self, data_path_sb, num_train_epochs, sampling_freq, sequence_len, device):
    ...
    def process_data(self):
        ...
        # For testing, use SB1, SB2, SB3, SB4 sequences (in order)
        ...
        self.test_seq1, self.test_labels1 = create_sequences_single_step(standardize_piece(self.sb1).values, self.sequence_len)

        self.test_seq2, self.test_labels2 = create_sequences_single_step(standardize_piece(self.sb2).values, self.sequence_len)
        self.test_seq3, self.test_labels3 = self.train_seq1, self.train_labels1
        self.test_seq4, self.test_labels4 = self.train_seq2, self.train_labels2
    ...
    def unnormalize(self, preds: torch.FloatTensor, ds_lbl: str):
        ...
        if ds_lbl == 'sb_dataset1':
            ...
            unnormed_labels = self.test_labels1_tensor.squeeze(1).cpu().numpy() * self.sb1_std + self.sb1_mean
        elif ds_lbl == 'sb_dataset2':
            y_hat = preds.numpy() * self.sb2_std + self.sb2_mean
            unnormed_labels = self.test_labels2_tensor.squeeze(1).cpu().numpy() * self.sb2_std + self.sb2_mean
        elif ds_lbl == 'sb_dataset3':
            y_hat = preds.numpy() * self.sb3_std + self.sb3_mean
            unnormed_labels = self.test_labels3_tensor.squeeze(1).cpu().numpy() * self.sb3_std + self.sb3_mean
        elif ds_lbl == 'sb_dataset4':
            y_hat = preds.numpy() * self.sb4_std + self.sb4_mean
            unnormed_labels = self.test_labels4_tensor.squeeze(1).cpu().numpy() * self.sb4_std + self.sb4_mean
        else:
