...
class CrossRegionFLSB():
    ...
    def process_data(self):
        ...
        # preprocess TEST Santa-Barbara data
        self.df_test = process_sb.make_timeseries(self.data_path_test)
    
        self.first_piece_test = process_sb.get_first_piece(self.df_test)
        self.first_piece_test_mean = self.first_piece_test.mean().values
        self.first_piece_test_std = self.first_piece_test.std().values
        self.second_piece_test = process_sb.get_second_piece(self.df_test)
        ...
        self.third_piece_test = process_sb.get_third_piece(self.df_test)
        self.third_piece_test_mean = self.third_piece_test.mean().values
        self.third_piece_test_std = self.third_piece_test.std().values
        self.fourth_piece_test = process_sb.get_fourth_piece(self.df_test)
        ...
        # create TEST (Santa-Barbara) sequences
        self.sb_seq1, self.sb_labels1 = create_sequences_single_step(standardize_piece(self.first_piece_test).values,
                                                                       self.sequence_len)
        self.sb_seq2, self.sb_labels2 = create_sequences_single_step(standardize_piece(self.second_piece_test).values,
                                                                      self.sequence_len)
        self.sb_seq3, self.sb_labels3 = create_sequences_single_step(standardize_piece(self.third_piece_test).values,
                                                                      self.sequence_len)
        self.sb_seq4, self.sb_labels4 = create_sequences_single_step(standardize_piece(self.fourth_piece_test).values,
                                                                      self.sequence_len)
        ...
        # create test tensors
        self.sb1_tensor = make_sequence_tensor(self.sb_seq1, self.device)
        self.sb_labels1_tensor = make_sequence_tensor(self.sb_labels1, self.device)
        self.sb2_tensor = make_sequence_tensor(self.sb_seq2, self.device)
        self.sb_labels2_tensor = make_sequence_tensor(self.sb_labels2, self.device)
    ...
    def unnormalize(self, preds: torch.FloatTensor, ds_lbl: str):
        ...
        preds = preds.cpu()
        if ds_lbl == 'sb_dataset1':
            y_hat = preds.numpy() * self.first_piece_test_std + self.first_piece_test_mean
            unnormed_labels = self.sb_labels1_tensor.squeeze(1).cpu().numpy() * self.first_piece_test_std + self.first_piece_test_mean
        elif ds_lbl == 'sb_dataset2':
            y_hat = preds.numpy() * self.second_piece_test_std + self.second_piece_test_mean
            unnormed_labels = self.sb_labels2_tensor.squeeze(1).cpu().numpy() * self.second_piece_test_std + self.second_piece_test_mean
        elif ds_lbl == 'sb_dataset3':
            y_hat = preds.numpy() * self.third_piece_test_std + self.third_piece_test_mean
            unnormed_labels = self.sb_labels3_tensor.squeeze(1).cpu().numpy() * self.third_piece_test_std + self.third_piece_test_mean
        elif ds_lbl == 'sb_dataset4':
            y_hat = preds.numpy() * self.fourth_piece_test_std + self.fourth_piece_test_mean
            unnormed_labels = self.sb_labels4_tensor.squeeze(1).cpu().numpy() * self.fourth_piece_test_std + self.fourth_piece_test_mean
        else:
    ...
    def set_test_data(self, test_only: bool=False):
        ...
        #     return True
        self.dataset_lbls = ['sb_dataset1', 'sb_dataset2', 'sb_dataset3', 'sb_dataset4']
        self.test_seqs = [self.sb1_tensor, self.sb2_tensor, self.sb3_tensor, self.sb4_tensor]
        self.test_lbls = [self.sb_labels1_tensor, self.sb_labels2_tensor, self.sb_labels3_tensor, self.sb_labels4_tensor]
