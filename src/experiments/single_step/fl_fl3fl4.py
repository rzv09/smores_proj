...
"""
class FederatedLearningFL3FL4():
    def __init__(self, data_path_fl, num_train_epochs, num_rounds,
    ...
    def process_data(self):
        ...
        self.train_seq1, self.train_labels1 = create_sequences_single_step(standardize_piece(self.fl3).values, self.sequence_len)
        self.train_seq2, self.train_labels2 = create_sequences_single_step(standardize_piece(self.fl4).values, self.sequence_len)
        ...
        self.test_seq1, self.test_labels1 = create_sequences_single_step(standardize_piece(self.fl1).values, self.sequence_len)
        # self.test_seq3, self.test_labels3 = create_sequences_single_step(standardize_piece(self.sb4).values, self.sequence_len)
        self.test_seq2, self.test_labels2 = create_sequences_single_step(standardize_piece(self.fl2).values, self.sequence_len)
    ...
    def unnormalize(self, preds: torch.FloatTensor, ds_lbl: str):
        ...
        if ds_lbl == 'fl_dataset3':
            y_hat = preds.numpy() * self.fl3_std + self.fl3_mean
            unnormed_labels = self.test_labels1_tensor.squeeze(1).cpu().numpy() * self.fl3_std + self.fl3_mean
        elif ds_lbl == 'fl_dataset1':
            y_hat = preds.numpy() * self.fl1_std + self.fl1_mean
            unnormed_labels = self.test_labels3_tensor.squeeze(1).cpu().numpy() * self.fl1_std + self.fl1_mean
        elif ds_lbl == 'fl_dataset4':
            y_hat = preds.numpy() * self.fl4_std + self.fl4_mean
            unnormed_labels = self.test_labels2_tensor.squeeze(1).cpu().numpy() * self.fl4_std + self.fl4_mean
    ...
    def set_test_data(self, model_lbl: str, test_only: bool=False):
        self.dataset_lbls = ['fl_dataset3', 'fl_dataset4', 'fl_dataset1', 'fl_dataset2']
